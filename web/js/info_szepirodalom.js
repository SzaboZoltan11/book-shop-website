document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadBooks(categoryId) {
        fetch('books.php?category_id=' + encodeURIComponent(categoryId))
            ...
            .then(data => {
                if (!data) return;
                const container = document.getElementById('szepirodalom-details');
                if (data.error) {
                ...
                // ⬇️ Eseménykezelők HOZZÁADÁSA a frissen betöltött elemekhez
                container.querySelectorAll(".buy-btn").forEach((button, index) => {
                ...
                container.querySelectorAll(".wishlist-icon").forEach((icon, index) => {
