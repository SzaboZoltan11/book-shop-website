...
if ($result->num_rows > 0) {
    ...
    $stmt = $conn->prepare($sql);
    if (!$stmt) {
        die("SQL hiba (INSERT): " . $conn->error);
    }
    $stmt->bind_param("ssss", $email, $token, $created_at, $expires);
    $stmt->execute();
    ...
    $message = "Klikkeljen ide a jelszó visszaállításához: <a href='" . $reset_link . "'>Jelszó visszaállítása</a>";
